...
async def parse_claude_response(response: AIMessage, pydantic_model: BaseModel):
    if isinstance(response.content, list):
        # Use thinking content if required.
...
async def generate_sysml_model(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    if "model_validation_response" not in state or state["model_validation_response"] is None:
        ...
        user_prompt = (
            "Generate SysML V2 updated model for the following requirements based on the provided plan and the "
            "previous model incorporating the feedback from the model validation agent.\n\n"
            "<Requirements>\n"
            f"{state['requirements']}\n"
            "</Requirements>\n\n"
            "<Plan>\n"
...
async def model_validation(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    # If Valid share the Generated Response to the End User.
    if model_validation_response.validation_outcome == "valid":
        return Command(
            goto=END,
            update={
                "messages": AIMessage(content=state["model_generation_response"].sysmlv2_notations)
            },
        )
    # If Invalid, generate the model again based on the feedback.
    if (
        model_validation_response.validation_outcome == "invalid"
        and state["current_iteration_count"] < state["no_of_allowed_feedback_iterations"]
    ):
        return Command(
            goto="generate_sysml_model",
            update={
                "current_iteration_count": state["current_iteration_count"] + 1,
                "model_validation_response": model_validation_response,
            },
        )
...
# We don't use conditional edge and instead use Command to route the flow along with the state update.
# Refer here for more information - https://langchain-ai.github.io/langgraph/concepts/low_level/#when-should-i-use-command-instead-of-conditional-edges
